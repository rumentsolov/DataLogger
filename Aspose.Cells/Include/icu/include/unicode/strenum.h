 ...
 * subsequent use.</p>
 *
 * <p>Strings may take the form of const char*, const UnChar*, or const
...
public:
     ...
     *
     * <p>If the native service string is a UnChar* string, it is
    ...
    /**
     * <p>Returns the next element as a NUL-terminated UnChar*.  If there
     * are no more elements, returns NULL.  If the resultLength pointer
     * is not NULL, the length of the string (not counting the
     * terminating NUL) is returned at that address.  If an error
     * status is returned, the value at resultLength is undefined.</p>
     ...
     * @stable ICU 2.4 
     */
    virtual const UnChar* unext(int32_t *resultLength, UErrorCode& status);
