...
private:
     ...
     *   create a matcher with a (UnChar *) string as input rather than
     *   a UnicodeString.  Avoids a dangling reference to a temporary string.
     * <p>
     * To efficiently work with UnChar *strings, wrap the data in a UnicodeString
     * using one of the aliasing constructors, such as
     * <code>UnicodeString(UBool isTerminated, const UnChar *text, int32_t textLength);</code>
     * or in a UText, using
     * <code>utext_openUChars(UText *ut, const UnChar *text, int64_t textLength, UErrorCode *status);</code>
     *
     */
    RegexMatcher *matcher(const UnChar *input,
        UErrorCode          &status) const;
public:
...
private:
     ...
     *
     */
    RegexMatcher(const UnicodeString &regexp, const UnChar *input,
        uint32_t flags, UErrorCode &status);
public:
...
private:
    /**
     * Cause a compilation error if an application accidentally attempts to
     *   reset a matcher with a (UnChar *) string as input rather than
     *   a UnicodeString.    Avoids a dangling reference to a temporary string.
     * <p>
     * To efficiently work with UnChar *strings, wrap the data in a UnicodeString
     * using one of the aliasing constructors, such as
     ...
     *
     */
    RegexMatcher &reset(const UnChar *input);
public:
